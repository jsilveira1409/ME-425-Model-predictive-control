classdef Rocket
    ...
    properties (Constant)
        ...
        ubx = [deg2rad([Inf Inf Inf, 7 7 Inf]), Inf Inf Inf, Inf Inf Inf]';
        lbx = -[deg2rad([Inf Inf Inf, 7 7 Inf]), Inf Inf Inf, Inf Inf Inf]';
    ...
    methods
        ...
        function mpc = merge_lin_controllers(obj, xs, us, mpc_x, mpc_y, mpc_z, mpc_r)
            ...
            function [u, T_opt, X_opt, U_opt] = merged_TXU_opt(z_, ref_)
                ...
                [u_y, ~,       X_opt_y, U_opt_y] = mpc_y.get_u( z_(idx_y) - xs(idx_y),              ref_(2) );
                [u_z, ~,       X_opt_z, U_opt_z] = mpc_z.get_u([z_(idx_z) - xs(idx_z); z_(13:end)], ref_(3) );
                [u_r, ~,       X_opt_r, U_opt_r] = mpc_r.get_u( z_(idx_r) - xs(idx_r),              ref_(4) );
                ...
                delta_X_opt = nan(12, size(X_opt_x, 2));
                delta_X_opt(idx_x,:) = X_opt_x;
                delta_X_opt(idx_y,:) = X_opt_y;
                delta_X_opt(idx_z,:) = X_opt_z;
                delta_X_opt(idx_r,:) = X_opt_r;
                X_opt = xs + delta_X_opt;
                
                delta_U_opt = Iu_x .* U_opt_x + Iu_y .* U_opt_y + Iu_z .* U_opt_z + Iu_r .* U_opt_r;
